<template>
  <BaseModal>
    ...
    <div
      ...
      :class="theme === 'light' ? 'share-profile__template-light' : 'share-profile__template-dark'"
    >
      <article class="share-profile__template-wrapper">
        ...
        <footer>&copy; Monody {{ new Date().getFullYear() }} — Tous droits réservés</footer>
      </article>
    </div>
...
if (window.matchMedia("(prefers-color-scheme: dark)") === false || window.matchMedia("(prefers-color-scheme: dark)").matches === false) {
	theme.value = "light";
}
...
const link = async () => {
	...
	await navigator.clipboard.writeText(`${location.origin}/assets/profiles/${store.id}.png`);
	alertStore.addAlerts({ "info": "Le lien vers le profil a été copié dans le presse-papiers" });
};
...
const image = async () => {
	...
	if (typeof ClipboardItem === "undefined") {
		await navigator.clipboard.writeText(`${location.origin}/assets/profiles/${store.id}.png`);
		alertStore.addAlerts({ "warn": "Votre navigateur ne supporte pas cette fonctionnalité" });
		alertStore.addAlerts({ "info": "Le lien vers le profil a été copié dans le presse-papiers" });
		return;
	}
	...
	alertStore.addAlerts({ "info": "Le profil a été copié dans le presse-papiers" });
};
