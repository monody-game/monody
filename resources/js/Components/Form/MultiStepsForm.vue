<template>
	<div class="auth-page__form-wrapper">
		<div v-if="props.loading" class="auth-page__loading-group">
		...
		<div class="auth-page__title-group">
			<h1>
				<slot name="title" :page="current" :total-page="props.pages" />
			</h1>
		</div>
		<form class="register-page__form" method="post" action="" @submit.prevent>
			<slot name="inputs" :page="current" />
...
const next = () => {
	current.value === Number.parseInt(props.pages)
		? emit("submit")
		: current.value++;
