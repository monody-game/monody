...
class StartGameController extends Controller
{
    public function check(GameIdRequest $request): JsonApiResponse
    {
        ...
        if (($game['type'] & GameType::NORMAL->value) === GameType::NORMAL->value && $this->isFull($game)) {
            return new JsonApiResponse(status: Status::NO_CONTENT);
        }

        if (($game['type'] & GameType::VOCAL->value) === GameType::VOCAL->value && $this->isFull($game) && $this->allUsersJoinedVoiceChannel($game)) {
            return new JsonApiResponse(status: Status::NO_CONTENT);
        }
