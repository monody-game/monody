...
final class EloService
{
     ...
     */
    public function computeElo(string $userId, string $gameId, bool $win = true): int
    {
        $eloList = $this->getGameElo($gameId)->map(fn ($model) => $model->elo);
        $maxElo = $eloList->max();
        $minElo = $eloList->min();
        $userElo = Elo::select(['elo'])->firstOrCreate(['user_id' => $userId]);
    ...
    {
        Elo::where('user_id', $userId)->increment('elo', $quantity);
    }
